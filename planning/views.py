...
class PlanningView(ViewSet):
  ...
  def get_sprints(self, request):
    return Response(self.service.get_sprints(), status=status.HTTP_200_OK)

  def get_unassigned_current_issues(self, request):
    return Response(self.service.get_unassigned_current_issues(), status=status.HTTP_200_OK)
